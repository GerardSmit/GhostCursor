...
public abstract class CefBrowserImpl(IWebBrowser browser) : BrowserBase<CefElement>
{
    ...
    public override async Task ScrollToAsync(Vector2 point, Random random, CefElement element,
        CancellationToken token = default)
    {
        ...
        while (Math.Abs(targetY) > randomOffset + 2)
        {
            if (attempts > 0)
            {
                var isAtBottom =
                    await ExecuteJsAsync(
                        "JSON.stringify(window.innerHeight + window.scrollY >= document.body.offsetHeight)", token);
            ...
            while (currentY != y && !token.IsCancellationRequested)
            {
                ...
                host.SendMouseWheelEvent((int)point.X, (int)point.Y, 0, isDown ? -scrollY : scrollY,
                    CefEventFlags.None);
    ...
    public override async Task ClickAsync(CefElement element, Vector2 point, int delay = 50,
        CancellationToken token = default)
    {
        var host = browser.GetBrowser().GetHost();
